...
(function (factory) {
  ...
  return (function () {
    ...
    return function (input) {
      const setUserIp = (ip) => {
        document
          .querySelectorAll("input[name=ip]")
          .forEach((item) => item.setAttribute("value", ip));
      };
      const setUserCountry = (country) => {
        document
          .querySelectorAll("input[name=country]")
          .forEach((item) => item.setAttribute("value", country));
      };

      const setFullNameCountry = (name) => {
        document
          .querySelectorAll("input[name=full-country]")
          .forEach((item) => item.setAttribute("value", name));
      };
      var iti = null;
      var arr = [];
      input.forEach((el) => {
        iti = new Iti(el, {
          utilsScript: "js/form/js/utils.js",
          nationalMode: true,
          defaultCountry: "auto",
          ...
          geoIpLookup: function (callback) {
            fetch("https://get.geojs.io/v1/ip/country.json")
              .then((data) => data.json())
              .then(({ country, ip, name }) => {
                callback(country);
                setUserIp(ip);
                setUserCountry(country);
                setFullNameCountry(name);
              });
          },
        });
        iti._init();
        el.setAttribute("data-intl-tel-input-id", iti.id);
